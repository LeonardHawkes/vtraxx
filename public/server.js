(function () {
	...
	var displayName = 'VTRAXX';
	var dateOptions = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
	var today = new Date();

	var userProfileSource = document.getElementById('user-profile-template').innerHTML,
		userProfileTemplate = Handlebars.compile(userProfileSource),
		userProfilePlaceholder = document.getElementById('disc');
		...
		$.ajax({
			...
			success: function (response) {
				...
				document.getElementById('download').addEventListener('click', function () {
					var offScreen = document.querySelector('.discContainer');
					...
					html2canvas(offScreen).then((canvas) => {
						...
						link.download = `${timeRangeSlug}_vtraxx.png`;
						link.href = dataURL;
	...
	if (error) {
		...
		document.getElementById('short_term').addEventListener('click', retrieveTracks('short_term', 1, 'PAST MONTH'), false);
